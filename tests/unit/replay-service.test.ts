...
import { createRun, createStepRun, completeStepRun, completeRun, getRunWithSteps } from "@opencode/engine/persistence/run-store.js"
...
describe("Replay Service", () => {
  ...
  it("replayRun creates a new run with replayOf metadata", async () => {
    ...
    const { replayRun } = await import("@opencode/engine/compiler/replay-service.js")
  ...
  it("replayFromStep replays from a specific step", async () => {
    ...
    const { replayFromStep } = await import("@opencode/engine/compiler/replay-service.js")
